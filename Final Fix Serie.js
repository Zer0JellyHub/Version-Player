...
if(!p){p=document.createElement('div');p.id='vsf-popup';document.body.appendChild(p);document.addEventListener('click',e=>{const b=document.getElementById('vsf-btn');if(!p.contains(e.target)&&!b?.contains(e.target))p.classList.remove('open')})}
