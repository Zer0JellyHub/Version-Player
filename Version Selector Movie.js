/* Jellyfin Version Selector - FILME */
(function () {
  'use strict';
  ...
  const CSS = `
    ...
    #vsf-popup .hdr { padding: 12px 16px; background: rgba(0,164,220,0.1); border-bottom: 1px solid #444; font-size: 11px; font-weight: 800; letter-spacing: 1.5px; text-transform: uppercase; color: #00a4dc; }
    #vsf-popup .list { max-height: 300px; overflow-y: auto; padding: 6px; }
    #vsf-popup .item { padding: 10px 12px; border-radius: 8px; cursor: pointer; border: 1px solid transparent; display: flex; align-items: center; justify-content: space-between; gap: 8px; color: #fff; transition: background 0.15s; }
    ...
    #vsf-popup .iname { font-size: 13px; font-weight: 600; margin-bottom: 2px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
    #vsf-popup .imeta { font-size: 10px; color: #888; }
    #vsf-popup .chk { width: 20px; height: 20px; border-radius: 50%; background: #00dc7d; display: flex; align-items: center; justify-content: center; font-size: 11px; flex-shrink: 0; }
    #vsf-popup .unchk { width: 20px; height: 20px; border-radius: 50%; border: 1.5px solid #555; flex-shrink: 0; }
    #vsf-popup .ftr { padding: 8px 16px; border-top: 1px solid #444; font-size: 10px; color: #666; text-align: center; }
    #vsf-toast { position: fixed; bottom: 140px; left: 50%; transform: translateX(-50%) translateY(10px); background: rgba(0,0,0,0.92); border: 1px solid rgba(0,220,125,0.45); color: #00dc7d; padding: 9px 18px; border-radius: 8px; font-size: 12px; font-weight: 700; z-index: 999999; opacity: 0; pointer-events: none; transition: all 0.3s ease; white-space: nowrap; }
  ...
  function tryInject() {
    ...
    if (_btnReady || document.getElementById('vsf-btn')) { _btnReady = true; return; }
    ...
    ApiClient.getItem(ApiClient.getCurrentUserId(), id).then(item => {
      ...
      if (document.getElementById('vsf-btn')) return;
      const c2 = document.querySelector('.buttons.focuscontainer-x');
      if (!c2) return;
      const btn = makeButton('vsf-btn');
