...
function captureAndGetId(){
  ...
  if(match && match[1]){
    localStorage.setItem('vsf_last_movie_id', match[1]);
    return match[1];
  }
  ...
  if(domEl){
    ...
    if(val){
      localStorage.setItem('vsf_last_movie_id', val);
      return val;
    }
  }
  return localStorage.getItem('vsf_last_movie_id');
}
...
async function switchVersion(msId){
  ...
  const id = captureAndGetId();
  
  if(!id || !vid){
    toast('⚠ Film-ID nicht ermittelbar');
    return;
  }
  ...
  try{
    ...
    const res = await fetch(`${base}/Items/${id}/PlaybackInfo?userId=${u}`, {
      method: 'POST',
      headers: {
    ...
    const url = src.SupportsDirectStream 
      ...
      : src.TranscodingUrl ? base + src.TranscodingUrl : `${base}/Videos/${id}/stream?MediaSourceId=${msId}&api_key=${tok}`;
...
async function openPopup(){
  ...
  if(!sources.length){
    try{
      ...
      if(item?.MediaSources) sources = item.MediaSources;
    }catch(err){}
  }

  if(!sources.length){
    toast('⚠ Keine Versionen verfügbar');
    return;
  }
  if(sources.length === 1){
    toast('ℹ Nur eine Version vorhanden');
    return;
  }
  ...
  p.innerHTML = `<div class="hdr">🎬 Film-Versionen <span style="opacity:.5;font-weight:400;letter-spacing:0;margin-left:6px">${sources.length} verfügbar</span></div><div class="list">${sources.map((s, i) => {
    const a = s.Id === activeId;
    const vs = (s.MediaStreams || []).find(x => x.Type === 'Video');
    const meta = [vs?.Height ? vs.Height + 'p' : '', s.Size ? (s.Size / 1073741824).toFixed(1) + ' GB' : ''].filter(Boolean).join(' · ');
    return `<div class="item${a ? ' active' : ''}" data-ms="${s.Id}"><div style="min-width:0"><div class="iname">${verName(s, i)}</div>${meta ? `<div class="imeta">${meta}</div>` : ''}</div>${a ? '<div class="chk">✓</div>' : '<div class="unchk"></div>'}</div>`;
  }).join('')}</div>`;
...
function getOrCreateBtn(){
  if(!_btn){
    ...
    _btn.title = 'Film-Versionen wählen';
...
function insertBtn(controls){
  if(_btn?.parentNode === controls) return;
...
function tryInject(){
  captureAndGetId();
  // Exakte Erkennung über das sichtbare OSD-Element gemäß Namo's Tipp
  const controls = document.querySelector('[data-type="video-osd"]:not(.hide) .buttons');
